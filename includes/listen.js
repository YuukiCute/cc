        ...
        if (day != day_now) {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    api.sendMessage({
    body: checkttBody, attachment: (await axios.get((await axios.get(`https://docs-api.catteam123.repl.co/images/girl?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
...
(async function () {
    ...
    try {
      ...
      for (const dataU of users) {
    	...
    	logger.loader(`BOT catteam123 BẮT ĐẦU NHẬN LỆNH`);
    } catch (error) {
...
░░░╚═╝░░░╚═╝░░╚═╝╚═╝░░╚═╝░╚════╝░░░░╚═╝░░░
`, "JRT");
  logger(`${global.config.ADMINBOT} - catteam123 is main!!!`, "ADMINBOT INFO");
  logger(`${global.config.NDH}`, "NDH INFO");
  logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] ➜ ${(!global.config.BOTNAME) ? "This bot was made by JRT" : global.config.BOTNAME}`, "BOT INFORMATION");
...
//DEFINE DATLICH PATH
  ...
  return async (event) => {
   if (event.type == "change_thread_image") api.sendMessage(`[⚜️]=== 『 ${global.config.BOTNAME} 』 ===[⚜️]\n━━━━━━━━━━━━━━━━\n${event.snippet}`, event.threadID);
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID) &&!ndhBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        adminBot.forEach(e => {
          api.sendMessage(`=== 『 DUYỆT BOX 』 ===\n━━━━━━━━━━━━━━━━\n[⚜️] ➜ Tên: ${threadName}\n[⚜️] ➜ ID: ${event.threadID}\n[⚜️] ➜ Đã yêu cầu được duyệt\n[⚜️]➜ Liên hệ: m.me/NHD.JRT.262\n━━━━━━━━━━━━━━━━\n===「${thu} || ${gio}」===`, e);
        })
        return api.sendMessage({
    body: `=== 『 REQUEST SUCCESS 』 ===\n━━━━━━━━━━━━━━━━\n[⚜️] ➜ Đã gửi yêu cầu đến ${admin.length} ADMINBOT\n━━━━━━━━━━━━━━━━\n===「${thu} || ${gio}」===`, attachment: (await axios.get((await axios.get(`https://docs-api.catteam123.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
}, event.threadID, () => {
    ...
    body: `=== 『 REQUEST 』 ===\n━━━━━━━━━━━━━━━━\n[⚜️] ➜ Nhóm của bạn chưa được duyệt sử dụng\n[⚜️] ➜ Để gửi yêu cầu duyệt hãy dùng: ${prefix}request !!!\n[⚜️]➜ Liên hệ: m.me/NHD.JRT.262\n━━━━━━━━━━━━━━━━\n===「${thu} || ${gio}」===`, attachment: (await axios.get((await axios.get(`https://docs-api.catteam123.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
}, event.threadID);
      		...
      		case "change_thread_image": 
        		if(global.config.notiGroup) {
					...
					if(event.author == api.getCurrentUserID()) {
						msg = msg.replace('[⚜️] ➜ Bạn', global.config.BOTNAME)
					}
					return api.sendMessage({
    body: `${msg}\n━━━━━━━━━━━━━━━━\n===「${thu} || ${gio}」===`, attachment: (await axios.get((await axios.get(`https://docs-api.catteam123.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
			...
			case "event":
				...
				handleRefresh({ event });
				if(global.config.notiGroup) {
					var msg = '[⚜️]=== 『 𝗖𝗔̣̂𝗣 𝗡𝗛𝗔̣̂𝗧 𝗡𝗛𝗢́𝗠 』 ===[⚜️]\n━━━━━━━━━━━━━━━━\n'
